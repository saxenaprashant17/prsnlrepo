trigger AggiornaCountOppGinecologo on Opportunity (after update) {
/*
    for(Opportunity opNew : Trigger.new)
    {
        ...
        if (opNew.Ginecologo__c != null)
        {
            ...
            {
                countOpp = [SELECT COUNT() FROM Opportunity WHERE Ginecologo__c = :idGinecologo AND RecordType.Name = 'SS' AND (Data_firma_contratto__c >= :data90GiorniFa)];
                countOppPerse = [SELECT COUNT() FROM Opportunity WHERE Ginecologo__c = :idGinecologo AND RecordType.Name = 'SS' AND ((StageName = 'Prospect perso') OR (StageName = 'Chiuso perso')) AND (Data_firma_contratto__c >= :data90GiorniFa)];
        ...
        if ((opOld.Ginecologo__c != null) && (opOld.Ginecologo__c != opNew.Ginecologo__c))
        {
